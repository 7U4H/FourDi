...
local requestsDisabled = true --getgenv and getgenv().DISABLE_FourDi_REQUESTS
local InterfaceBuild = '3K3W'
local Release = "Build 1.672"
local FourDiFolder = "FourDi"
local ConfigurationFolder = FourDiFolder.."/Configurations"
local ConfigurationExtension = ".rfld"
local settingsTable = {
	General = {
		-- if needs be in order just make getSetting(name)
		FourDiOpen = {Type = 'bind', Value = 'K', Name = 'FourDi Keybind'},
		-- buildwarnings
		-- FourDiprompts
...
local function loadSettings()
	...
	local success, result =	pcall(function()
		task.spawn(function()
			if isfolder and isfolder(FourDiFolder) then
				if isfile and isfile(FourDiFolder..'/settings'..ConfigurationExtension) then
					file = readfile(FourDiFolder..'/settings'..ConfigurationExtension)
				end
			end
			...
			if useStudio then
				file = [[
		{"General":{"FourDiOpen":{"Value":"K","Type":"bind","Name":"FourDi Keybind","Element":{"HoldToInteract":false,"Ext":true,"Name":"FourDi Keybind","Set":null,"CallOnChange":true,"Callback":null,"CurrentKeybind":"K"}}},"System":{"usageAnalytics":{"Value":false,"Type":"toggle","Name":"Anonymised Analytics","Element":{"Ext":true,"Name":"Anonymised Analytics","Set":null,"CurrentValue":false,"Callback":null}}}}
	]]
			end
	...
	if not success then 
		if writefile then
			warn('FourDi had an issue accessing configuration saving capability.')
		end
	end
end
		...
		if useStudio then
			...
			task.spawn(function()
				...
				if success and reporter then
					pcall(function()
						reporter.report("FourDi", Release, InterfaceBuild)
					end)
				else
					warn("Failed to load or execute the reporter. \nPlease notify FourDi developers at sirius.menu/discord.")
				end
			end)
...
local FourDi = useStudio and script.Parent:FindFirstChild('FourDi') or game:GetObjects("rbxassetid://10804731440")[1]
...
repeat
	if FourDi:FindFirstChild('Build') and FourDi.Build.Value == InterfaceBuild then
		correctBuild = true
		break
	end
	...
	if not warned then
		warn('FourDi | Build Mismatch')
		print('FourDi may encounter issues as you are running an incompatible interface version ('.. ((FourDi:FindFirstChild('Build') and FourDi.Build.Value) or 'No Build') ..').\n\nThis version of FourDi is intended for interface build '..InterfaceBuild..'.')
		warned = true
	end

	toDestroy, FourDi = FourDi, useStudio and script.Parent:FindFirstChild('FourDi') or game:GetObjects("rbxassetid://10804731440")[1]
	if toDestroy and not useStudio then toDestroy:Destroy() end

	buildAttempts = buildAttempts + 1
until buildAttempts >= 2

FourDi.Enabled = false

if gethui then
	FourDi.Parent = gethui()
elseif syn and syn.protect_gui then 
	syn.protect_gui(FourDi)
	FourDi.Parent = CoreGui
elseif not useStudio and CoreGui:FindFirstChild("RobloxGui") then
	FourDi.Parent = CoreGui:FindFirstChild("RobloxGui")
...
if gethui then
	for _, Interface in ipairs(gethui():GetChildren()) do
		if Interface.Name == FourDi.Name and Interface ~= FourDi then
			Interface.Enabled = false
			Interface.Name = "FourDi-Old"
		end
	end
elseif not useStudio then
...
if FourDi.AbsoluteSize.X < minSize.X and FourDi.AbsoluteSize.Y < minSize.Y then
...
local Main = FourDi.Main
local MPrompt = FourDi:FindFirstChild('Prompt')
local Topbar = Main.Topbar
local Elements = Main.Elements
local LoadingFrame = Main.LoadingFrame
local TabList = Main.TabList
local dragBar = FourDi:FindFirstChild('Drag')
...
FourDi.DisplayOrder = 100
LoadingFrame.Version.Text = Release

-- Thanks to Latte Softworks for the Lucide integration for Roblox
local Icons = useStudio and require(script.Parent.icons) or loadWithTimeout('https://raw.githubusercontent.com/SiriusSoftwareLtd/FourDi/refs/heads/main/icons.lua')
-- Variables
...
local Notifications = FourDi.Notifications

local SelectedTheme = FourDiLibrary.Theme.Default

local function ChangeTheme(Theme)
	if typeof(Theme) == 'string' then
		SelectedTheme = FourDiLibrary.Theme[Theme]
	elseif typeof(Theme) == 'table' then
		SelectedTheme = Theme
	end

	FourDi.Main.BackgroundColor3 = SelectedTheme.Background
	FourDi.Main.Topbar.BackgroundColor3 = SelectedTheme.Topbar
	FourDi.Main.Topbar.CornerRepair.BackgroundColor3 = SelectedTheme.Topbar
	FourDi.Main.Shadow.Image.ImageColor3 = SelectedTheme.Shadow

	FourDi.Main.Topbar.ChangeSize.ImageColor3 = SelectedTheme.TextColor
	FourDi.Main.Topbar.Hide.ImageColor3 = SelectedTheme.TextColor
	FourDi.Main.Topbar.Search.ImageColor3 = SelectedTheme.TextColor
	if Topbar:FindFirstChild('Settings') then
		FourDi.Main.Topbar.Settings.ImageColor3 = SelectedTheme.TextColor
		FourDi.Main.Topbar.Divider.BackgroundColor3 = SelectedTheme.ElementStroke
	end
	...
	for _, text in ipairs(FourDi:GetDescendants()) do
...
local function getAssetUri(id: any): string
	...
	elseif type(id) == "string" and not Icons then
		warn("FourDi | Cannot use Lucide icons as icons library is not loaded")
	else
		warn("FourDi | The icon argument must either be an icon ID (number) or a Lucide icon name (string)")
	end
	return assetUri
end
...
local function LoadConfiguration(Configuration)
	...
	if not success then warn('FourDi had an issue decoding the configuration file, please try delete the file and reopen FourDi.') return end

	-- Iterate through current UI elements' flags
	for FlagName, Flag in pairs(FourDiLibrary.Flags) do
		...
		if (typeof(FlagValue) == 'boolean' and FlagValue == false) or FlagValue then
			...
			warn("FourDi | Unable to find '"..FlagName.. "' in the save file.")
			print("The error above may not be an issue if new elements have been added or not been set values.")
			--FourDiLibrary:Notify({Title = "FourDi Flags", Content = "FourDi was unable to find '"..FlagName.. "' in the save file. Check sirius.menu/discord for help.", Image = 3944688398})
		end
	end
...
local function SaveConfiguration()
	...
	local Data = {}
	for i, v in pairs(FourDiLibrary.Flags) do
...
function FourDiLibrary:Notify(data) -- action e.g open messages
	task.spawn(function()
		...
		if data.Actions then
			warn('FourDi | Not seeing your actions in notifications?')
...
local function Hide(notify: boolean?)
	...
	if notify then
		if useMobilePrompt then 
			FourDiLibrary:Notify({Title = "Interface Hidden", Content = "The interface has been hidden, you can unhide the interface by tapping 'Show FourDi'.", Duration = 7, Image = 4400697855})
		else
			FourDiLibrary:Notify({Title = "Interface Hidden", Content = `The interface has been hidden, you can unhide the interface by tapping {settingsTable.General.FourDiOpen.Value or 'K'}.`, Duration = 7, Image = 4400697855})
		end
	end
...
local function updateSettings()
	...
	if success then
		...
		if writefile then
			writefile(FourDiFolder..'/settings'..ConfigurationExtension, encoded)
		end
	end
end
...
local function createSettings(window)
	...
	local newTab = window:CreateTab('FourDi Settings', 0, true)

	if TabList['FourDi Settings'] then
		TabList['FourDi Settings'].LayoutOrder = 1000
	end

	if Elements['FourDi Settings'] then
		Elements['FourDi Settings'].LayoutOrder = 1000
	end
...
function FourDiLibrary:CreateWindow(Settings)
	if FourDi:FindFirstChild('Loading') then
		if getgenv and not getgenv().FourDiCached then
			FourDi.Enabled = true
			FourDi.Loading.Visible = true

			task.wait(1.4)
			FourDi.Loading.Visible = false
		end
	end

	if getgenv then getgenv().FourDiCached = true end

	if not correctBuild and not Settings.DisableBuildWarnings then
		task.delay(3, 
			function() 
				FourDiLibrary:Notify({Title = 'Build Mismatch', Content = 'FourDi may encounter issues as you are running an incompatible interface version ('.. ((FourDi:FindFirstChild('Build') and FourDi.Build.Value) or 'No Build') ..').\n\nThis version of FourDi is intended for interface build '..InterfaceBuild..'.\n\nTry rejoining and then run the script twice.', Image = 4335487866, Duration = 15})		
			end)
	end

	if isfolder and not isfolder(FourDiFolder) then
		makefolder(FourDiFolder)
	end
	...
	LoadingFrame.Title.Text = Settings.LoadingTitle or "FourDi"
	LoadingFrame.Subtitle.Text = Settings.LoadingSubtitle or "Interface Suite"

	if Settings.LoadingTitle ~= "FourDi Interface Suite" then
		LoadingFrame.Version.Text = "FourDi UI"
	end
	...
	if not Settings.DisableFourDiPrompts then
		task.spawn(function()
			while true do
				task.wait(math.random(180, 600))
				FourDiLibrary:Notify({
					Title = "FourDi Interface",
	...
	if Settings.Discord and not useStudio then
		if isfolder and not isfolder(FourDiFolder.."/Discord Invites") then
			makefolder(FourDiFolder.."/Discord Invites")
		end

		if isfile and not isfile(FourDiFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension) then
			if request then
				pcall(function()
					request({
			...
			if Settings.Discord.RememberJoins then -- We do logic this way so if the developer changes this setting, the user still won't be prompted, only new users
				writefile(FourDiFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension,"FourDi RememberJoins is true for this invite, this invite will not ask you to join again")
			end
		end
	end
	...
	if (Settings.KeySystem) then
		...
		if isfolder and not isfolder(FourDiFolder.."/Key System") then
			makefolder(FourDiFolder.."/Key System")
		end
		...
		if Settings.KeySettings.GrabKeyFromSite then
			for i, Key in ipairs(Settings.KeySettings.Key) do
				...
				if not Success then
					print("FourDi | "..Key.." Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with FourDi specific development.')
				end
			end
		end
		...
		if isfile and isfile(FourDiFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension) then
			for _, MKey in ipairs(Settings.KeySettings.Key) do
				if string.find(readfile(FourDiFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension), MKey) then
					Passthrough = true
				end
			end
		end
			...
			KeyUI.Main.Input.InputBox.FocusLost:Connect(function()
				...
				if KeyFound then 
					...
					if Settings.KeySettings.SaveKey then
						if writefile then
							writefile(FourDiFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension, FoundKey)
						end
						FourDiLibrary:Notify({Title = "Key System", Content = "The key for this script has been saved successfully.", Image = 3605522284})
					end
				else
	...
	function Window:CreateTab(Name, Image, Ext)
		...
		function Tab:CreateButton(ButtonSettings)
			...
			Button.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("FourDi | "..ButtonSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with FourDi specific development.')
					task.wait(0.5)
					Button.Title.Text = ButtonSettings.Name
		...
		function Tab:CreateColorPicker(ColorPickerSettings) -- by Throit
			...
			ColorPicker.Interact.MouseButton1Down:Connect(function()
				...
				if not opened then
					...
					TweenService:Create(Main, TweenInfo.new(0.2, Enum.EasingStyle.Exponential), {ImageTransparency = SelectedTheme ~= FourDiLibrary.Theme.Default and 0.25 or 0.1}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and ColorPickerSettings.Flag then
					FourDiLibrary.Flags[ColorPickerSettings.Flag] = ColorPickerSettings
				end
			end
			...
			FourDi.Main:GetPropertyChangedSignal('BackgroundColor3'):Connect(function()
				for _, rgbinput in ipairs(ColorPicker.RGB:GetChildren()) do
					if rgbinput:IsA("Frame") then
						rgbinput.BackgroundColor3 = SelectedTheme.InputBackground
						rgbinput.UIStroke.Color = SelectedTheme.InputStroke
		...
		function Tab:CreateInput(InputSettings)
			...
			Input.InputFrame.InputBox.FocusLost:Connect(function()
				...
				if not Success then
					...
					print("FourDi | "..InputSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with FourDi specific development.')
					task.wait(0.5)
					Input.Title.Text = InputSettings.Name
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and InputSettings.Flag then
					FourDiLibrary.Flags[InputSettings.Flag] = InputSettings
				end
			end
		...
		function Tab:CreateDropdown(DropdownSettings)
				...
				for _, Option in ipairs(DropdownSettings.Options) do
					...
					DropdownOption.Interact.MouseButton1Click:Connect(function()
						...
						if not Success then
							...
							print("FourDi | "..DropdownSettings.Name.." Callback Error " ..tostring(Response))
							warn('Check docs.sirius.menu for help with FourDi specific development.')
							task.wait(0.5)
							Dropdown.Title.Text = DropdownSettings.Name
							TweenService:Create(Dropdown, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and DropdownSettings.Flag then
					FourDiLibrary.Flags[DropdownSettings.Flag] = DropdownSettings
				end
			end
		...
		function Tab:CreateKeybind(KeybindSettings)
				...
				elseif not KeybindSettings.CallOnChange and KeybindSettings.CurrentKeybind ~= nil and (input.KeyCode == Enum.KeyCode[KeybindSettings.CurrentKeybind] and not processed) then -- Test
					...
					if not KeybindSettings.HoldToInteract then
						...
						if not Success then
							...
							print("FourDi | "..KeybindSettings.Name.." Callback Error " ..tostring(Response))
							warn('Check docs.sirius.menu for help with FourDi specific development.')
							task.wait(0.5)
							Keybind.Title.Text = KeybindSettings.Name
							TweenService:Create(Keybind, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and KeybindSettings.Flag then
					FourDiLibrary.Flags[KeybindSettings.Flag] = KeybindSettings
				end
			end
		...
		function Tab:CreateToggle(ToggleSettings)
			...
			if SelectedTheme ~= FourDiLibrary.Theme.Default then
			...
			Toggle.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("FourDi | "..ToggleSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with FourDi specific development.')
					task.wait(0.5)
					Toggle.Title.Text = ToggleSettings.Name
					TweenService:Create(Toggle, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if not ToggleSettings.Ext then
				if Settings.ConfigurationSaving then
					if Settings.ConfigurationSaving.Enabled and ToggleSettings.Flag then
						FourDiLibrary.Flags[ToggleSettings.Flag] = ToggleSettings
					end
				end
			end
		...
		function Tab:CreateSlider(SliderSettings)
					...
					if SLDragging then
						...
						if SliderSettings.CurrentValue ~= NewValue then
							...
							if not Success then
								...
								print("FourDi | "..SliderSettings.Name.." Callback Error " ..tostring(Response))
								warn('Check docs.sirius.menu for help with FourDi specific development.')
								task.wait(0.5)
								Slider.Title.Text = SliderSettings.Name
								TweenService:Create(Slider, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and SliderSettings.Flag then
					FourDiLibrary.Flags[SliderSettings.Flag] = SliderSettings
				end
			end
	...
	function Window.ModifyTheme(NewTheme)
		...
		if not success then
			FourDiLibrary:Notify({Title = 'Unable to Change Theme', Content = 'We are unable find a theme on file.', Image = 4400704299})
		else
			FourDiLibrary:Notify({Title = 'Theme Changed', Content = 'Successfully changed theme to '..(typeof(NewTheme) == 'string' and NewTheme or 'Custom Theme')..'.', Image = 4483362748})
		end
	end
	...
	if not success then warn('FourDi had an issue creating settings.') end
...
function FourDiLibrary:SetVisibility(visibility: boolean)
	setVisibility(visibility, false)
end

function FourDiLibrary:IsVisible(): boolean
	return not Hidden
end

local hideHotkeyConnection -- Has to be initialized here since the connection is made later in the script
function FourDiLibrary:Destroy()
...
if Topbar:FindFirstChild('Settings') then
	Topbar.Settings.MouseButton1Click:Connect(function()
		task.spawn(function()
			...
			Elements.UIPageLayout:JumpTo(Elements['FourDi Settings'])
		end)
	end)
...
hideHotkeyConnection = UserInputService.InputBegan:Connect(function(input, processed)
	if (input.KeyCode == Enum.KeyCode[settingsTable.General.FourDiOpen.Value or 'K'] and not processed) then
		if Debounce then return end
		if Hidden then
			Hidden = false
			Unhide()
...
function FourDiLibrary:LoadConfiguration()
	local config
	...
	if CEnabled then
		...
		local success, result = pcall(function()
			...
			if isfile then 
				...
				notified = true
				FourDiLibrary:Notify({Title = "FourDi Configurations", Content = "We couldn't enable Configuration Saving as you are not using software with filesystem support.", Image = 4384402990})
			end
		end)

		if success and loaded and not notified then
			FourDiLibrary:Notify({Title = "FourDi Configurations", Content = "The configuration file for this script has been loaded from a previous session.", Image = 4384403532})
		elseif not success and not notified then
			warn('FourDi Configurations Error | '..tostring(result))
			FourDiLibrary:Notify({Title = "FourDi Configurations", Content = "We've encountered an issue loading your configuration correctly.\n\nCheck the Developer Console for more information.", Image = 4384402990})
		end
	end
...
if useStudio then
	...
	local Window = FourDiLibrary:CreateWindow({
		Name = "FourDi Example Window",
		LoadingTitle = "FourDi Interface Suite",
		Theme = 'Default',
		Icon = 0,
		...
		KeySettings = {
			...
			FileName = "Key", -- It is recommended to use something unique as other scripts using FourDi may overwrite your key file
			SaveKey = true, -- The user's key will be saved, but if you change the key, they will be unable to use your script
			GrabKeyFromSite = false, -- If this is true, set Key below to the RAW site you would like FourDi to get the key from
	...
	--FourDiLibrary:Notify({Title = "FourDi Interface", Content = "Welcome to FourDi. These - are the brand new notification design for FourDi, with custom sizing and FourDi calculated wait times.", Image = 4483362458})
	...
	local thoptions = {}
	for themename, theme in pairs(FourDiLibrary.Theme) do
...
task.delay(4, function()
	FourDiLibrary.LoadConfiguration()
